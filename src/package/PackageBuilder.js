...
class PackageBuilder {
   ...
   * @param {object} [validationReport] - v7.49: ValidationReport from independent validation
   * @returns {Promise<string>} - Path to created package
   */
  async build(encodedCells, reachMap, outputPath, sourceInfo = {}, validationReport = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // 1. Add manifest.json
      const manifest = this.createManifest(reachMap, sourceInfo, validationReport);
   ...
   * @param {object} [validationReport] - v7.49: ValidationReport instance
   * @returns {object}
   */
  createManifest(reachMap, sourceInfo, validationReport = null) {
    // v7.49: Use independently computed values if validation report available,
    // otherwise fall back to specification thresholds
    const inferenceContract = validationReport && typeof validationReport.toManifest === 'function'
      ? validationReport.toManifest()
      : {
