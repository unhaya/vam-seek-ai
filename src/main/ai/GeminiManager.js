...
class GeminiManager {
  ...
  async uploadVideo(videoPath) {
    ...
    // v7.38: Anonymize filename to prevent content inference from filename
    // Gemini may adjust filtering based on display_name
    ...
    const initResponse = await fetch(initUrl, {
      ...
      body: JSON.stringify({
        file: { display_name: anonymizedName }
      })
    });
...
[05:45] ハイライト - 重要な場面`;
  ...
  async uploadAudio(audioPath) {
    ...
    // v7.38: Anonymize audio filename too
    const anonymizedName = `audio_${Date.now()}.mp3`;

    console.log(`[GeminiManager] Uploading audio: ${originalFileName} as ${anonymizedName} (${(fileSize / 1024).toFixed(1)} KB)`);
...
【タイムスタンプの基準】
タイムスタンプ [MM:SS] = この音声ファイル内の実際の再生位置。
ナレーションや実況で言及される時刻（ミッション時刻等）ではない。
このタイムスタンプは動画のシーク操作に使われる。映像と対応する必要がある。
ナレーション以外の区間（インタビュー・BGM・無音）もスキップせずタイムスタンプに含めること。
スキップすると後続のタイムスタンプが前にずれて映像と一致しなくなる。
最後のタイムスタンプは音声の末尾付近であること。
...
3. 繰り返しは時間範囲 [05:00-05:30] でまとめる`
  ...
  // v7.48: Transcript injected on-demand from cache, not from conversation history
  // ============================================
  async analyzeGrid(userMessage, gridData, conversationHistory = [], learnedRulesPrompt = '', cachedTranscript = null) {
    if (!this.isConfigured()) {
      throw new Error('Gemini API key not configured');
    }
  ...
  buildGridSystemPrompt(gridData, learnedRulesPrompt = '', hasTranscript = false) {
    ...
    // v7.47: Added time uncertainty note to resolve double bind
    let prompt = `${durationMin}分の動画。${totalCells}セル、15秒間隔。
${processorPrompt}

【時間精度】各セルの時間被覆は可変（1.0〜6.5秒）。タイムスタンプは近似値、±3秒の誤差あり。正確な秒数が必要ならZOOM_REQUESTを使え。
