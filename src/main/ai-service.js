...
// v7.48: Transcript cache - stored separately from conversation history
// Transcript is injected on-demand per turn, not kept in history
...
function clearConversation() {
  ...
  cachedTranscript = null;  // v7.48: Clear transcript cache
  currentPhase = 'normal';
...
// Keeps: first message (with images) + last N exchanges
// This prevents token explosion while maintaining context
// v7.48: Transcript no longer in conversation history — no protection needed
const MAX_HISTORY_EXCHANGES = 4;  // Keep 4 exchanges (8 messages) max after first

function pruneConversationHistory() {
  // Need at least first exchange + MAX exchanges + 1 extra to prune
  ...
  // Build list of indices to remove (middle messages)
  const toRemove = [];
  ...
  if (toRemove.length > 0) {
    console.log(`[AI] Pruned ${toRemove.length} messages from conversation history`);
  }
}
...
function buildStaticSystemPrompt(gridData) {
  ...
  // v7.44: VAM-RGB specific prompt section
  let vamRgbSection = '';
  if (processorName.includes('VAM-RGB')) {
    vamRgbSection = `
【VAM-RGB時間エンコーディング v3.2】
この画像はVAM-RGB v3.2で生成されている。各ピクセルは3時点の情報を含む:
- Rチャンネル = T-0.5秒（過去）の8×8ブロック平均（モザイク）
- Gチャンネル = T（現在）+ 色復元ヒント（G-Nudge勾配）― per-pixel
- Bチャンネル = T+0.5秒（未来）の8×8ブロック平均（モザイク）

【G-Nudge (ψ3.1)】
Gチャンネルは8×8ブロック内に色差ヒントを勾配として埋め込んでいる:
- 水平方向の明暗変化 = R-G色差（右が明るい→赤み、暗い→緑み）
- 垂直方向の明暗変化 = B-G色差（下が明るい→青み、暗い→黄み）
- ブロック中心はG値がそのまま保存される

【R/B Mosaic (ψ3.2)】
R/Bチャンネルは8×8ブロック平均値（モザイク）で記録されている。
各8×8エリアが均一な値 = そのエリアのPast/Future平均輝度。
R_block ≈ B_block → 静止（時間変化なし）
R_block ≠ B_block → 時間変化あり
Gチャンネルは従来通りper-pixelで現在フレームの詳細を保持。

【動き検出】
- 静止シーン: RGB一致 → グレー/自然色
- 動きあり: RGB分離 → 色収差（赤/青のにじみ）
- 高速動作: 強い色分離 → 虹色の縁取り

1セルで0.5秒前後の動きを観測できる。色のズレが大きいセルは動作が激しい区間。

同じ言葉が続くと読みにくい。変化点だけ拾って、似た動作は時間範囲でまとめると...
...
function buildDynamicContext(gridData = null) {
  ...
  // v7.48: Check transcript cache instead of conversation history
  ...
  if (hasTranscript) {
    context += `【注意】音声文字起こし済み。[AUDIO_REQUEST]不要。\n`;
  }
...
async function analyzeGridGemini(userMessage, gridData) {
  ...
  try {
    ...
    // v7.48: Pass cached transcript for on-demand injection
    const learnedRulesPrompt = getLearnedRulesPrompt();
    const response = await geminiManager.analyzeGrid(userMessage, gridData, conversationHistory, learnedRulesPrompt, cachedTranscript);
  ...
  if (gridData?.cellMetadata && isFirstMessage) {
    try {
      const physicsProfiles = gridData.cellMetadata
        ...
        .map(m => m.physicsProfile);

      if (physicsProfiles.length > 0) {
        const va = new VerbalizationAnalyzer({
          secondsPerCell: gridData.secondsPerCell || 15
        });
        const verbalizationProfile = va.analyze(aiMessage, physicsProfiles);

        const cv = new CrossValidator({
          secondsPerCell: gridData.secondsPerCell || 15
        });
        const report = cv.validate(physicsProfiles, verbalizationProfile);
...
async function analyzeZoomGrid(userMessage, zoomGridData) {
  ...
  // v7.37: "Letter" style prompt - communicate intent, not just data
  const systemPrompt = `これは${zoomStart}～${zoomEnd}の拡大画像です。ユーザーは正確な時刻を知りたがっています。各セル左下のタイムスタンプを読み取ってください。`;
...
async function analyzeAudio(videoPath, options = {}) {
  ...
  try {
    ...
    // v7.48: Store transcript in cache (not conversation history)
    // Transcript is injected on-demand per turn to avoid bloating history
    cachedTranscript = result.text;
    console.log('[AI] Audio transcript cached (not in conversation history)');
...
// v7.48: Restore transcript to cache (not conversation history)
function restoreTranscript(transcriptText) {
  if (!transcriptText) return;

  cachedTranscript = transcriptText;
  console.log('[AI] Transcript restored to cache (not in conversation history)');
}
