...
class BaseGridProcessor {
  ...
  /**
   * V7.5: Get color separation for current frame (override in subclass)
   * @returns {number} 0.0-1.0
   */
  getLastColorSeparation() {
    return 0;  // Base class has no color separation
  }
  ...
  async generateGrid() {
    ...
    try {
      ...
      const gridImages = [];
      const timestamps = [];
      const cellMetadata = [];  // V7.5: Metadata per cell
      ...
      for (let imgIdx = 0; imgIdx < imageCount; imgIdx++) {
        ...
        for (let cellIdx = startCell; cellIdx < endCell; cellIdx++) {
          ...
          // V7.5: Collect cell metadata
          const meta = {
            index: cellIdx,
            timestamp: timestamp,
            timestampFormatted: this._formatTime(timestamp),
            deltaT: 0.5,  // Fixed stride for VAM-RGB v3.0
            colorSeparation: this.getLastColorSeparation(),
            audioEnergy: 0  // Will be filled by AudioReachDetector
          };
          ...
          if (false && typeof window !== 'undefined' && window.PhysicsAnalyzer && this.getLastImageData) {
            ...
            if (imageData) {
              try {
                const pa = new window.PhysicsAnalyzer();
                const profile = pa.analyze(imageData, cellIdx, timestamp);
                meta.physicsProfile = profile;
              } catch (e) {
                console.warn('[Validation] PhysicsAnalyzer error:', e.message);
              }
            }
          }

          cellMetadata.push(meta);
        }
      ...
      return {
        ...
        cellMetadata: cellMetadata  // V7.5: Cell metadata array
      };
