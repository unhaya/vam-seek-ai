...
window.electronAPI.onGridCaptureRequest(async () => {
  ...
  const gridData = {
    ...
    cellMetadata: aiGrid.cellMetadata     // V7.5: Structured metadata per cell
  };

  // V7.5: Apply critical cell overlays to human UI grid
  if (aiGrid.cellMetadata && vamInstance && typeof vamInstance.markCriticalCells === 'function') {
    const criticalCells = detectCriticalCellsLocal(aiGrid.cellMetadata);
    if (criticalCells.length > 0) {
      vamInstance.markCriticalCells(criticalCells);
      console.log(`[V7.5] Marked ${criticalCells.length} critical cells in UI`);
    }
  }
...
async function captureZoomGridForAI(startTime, endTime) {
  ...
  console.log(`[captureZoomGridForAI] First timestamp will be: ${formatTime(startTime)}, Last: ${formatTime(startTime + (totalCells - 1) * secondsPerCell)}`);
...
window.electronAPI.onZoomGridCaptureRequest(async (startTime, endTime) => {
  console.log(`[ZoomCapture] Request received: ${startTime}s - ${endTime}s`);

  if (!video.duration || video.readyState < 2) {
    console.log('[ZoomCapture] Video not ready');
  ...
  try {
    ...
    if (!zoomGrid) {
      console.log('[ZoomCapture] captureZoomGridForAI returned null');
      window.electronAPI.sendZoomGridCaptureResponse(null);
      return;
    }
    console.log(`[ZoomCapture] Success: ${zoomGrid.totalCells} cells`);
  ...
  const gridData = {
    ...
    console.error('[ZoomCapture] Exception:', err);
    window.electronAPI.sendZoomGridCaptureResponse(null);
  }
});
