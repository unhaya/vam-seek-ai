...
def extract_audio_ai_optimized(video_path: str, output_path: str) -> str:
    ...
    v7.44: Added noise cancellation filter to reduce token consumption.
           - highpass=200Hz: Remove wind/footsteps
           - lowpass=3000Hz: Remove metallic/air noise
           - silenceremove: Remove silent sections
    """
    # v7.44: Audio filter for noise cancellation
    audio_filter = (
        'highpass=f=200,'           # Cut low frequency noise (wind, footsteps)
        'lowpass=f=3000,'           # Cut high frequency noise (metallic, air)
        'silenceremove='
        'stop_periods=-1:'          # Remove all silent sections
        'stop_duration=1:'          # Minimum 1 second silence
        'stop_threshold=-30dB'      # Silence threshold
    )

    cmd = [
        'ffmpeg', '-y', '-i', video_path,
        '-vn',           # No video
        '-af', audio_filter,  # v7.44: Noise cancellation
...
def generate_ai_assets(video_path: str, output_dir: str, interval: float = 15.0,
    ...
    # v7.44: Audio filter for noise cancellation
    audio_filter = (
        'highpass=f=200,'           # Cut low frequency noise
        'lowpass=f=3000,'           # Cut high frequency noise
        'silenceremove='
        'stop_periods=-1:'
        'stop_duration=1:'
        'stop_threshold=-30dB'
    ...
    cmd = [
        'ffmpeg', '-y', '-i', video_path,
        # Audio output: AI-optimized MP3 with noise cancellation
        '-vn', '-af', audio_filter, '-ar', '16000', '-ac', '1', '-b:a', '48k', audio_output,
